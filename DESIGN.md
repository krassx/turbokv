...
- **The primary keeps a per-writer count of generations opened and not
  settled**, and reconciles what a worker still owed when its channel emits
  `'exit'` or `'disconnect'`. `install()` wires that; `TurboKV.releaseWorker(id)`
  is public for a primary that routes cluster messages itself, for the same
  reason `applyBatch` is.
...
someone else's clear was removing. Counting per writer turns the lever into an
accounting rule — a worker may settle exactly as many generations as it opened,
