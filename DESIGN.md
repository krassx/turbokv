...
promise chain that decrements the local counter also closes the generation. The
honest residue: a process `SIGKILL`ed between opening a generation and settling
it leaves the guard armed until a new primary recreates the arena. That is the
conservative direction (misses, never resurrection), and it is the truth about
that clear — it genuinely never landed.
