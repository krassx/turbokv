...
```js
const { Cache } = require('@krassx/turbokv');   // alias of TurboKV
...
| Primary crashes | **Handled.** Workers see the heartbeat go stale, detach (mandatory — on Windows a held handle blocks a new primary from creating the segment at all), keep serving their warm L1, and poll the name. They re-attach only when a heartbeat *advances* across two polls, then flush L1, reset the invalidation cursor and re-claim a ring slot. `arenaId` distinguishes a new primary from the same one resuming. **A worker notices on its next OPERATION** — `#checkPrimary` runs inside `#drain`, and workers have no timer of their own (`#startMaintenance` returns immediately for a non-zero id) — so an idle worker keeps its handle indefinitely. Elapsed time alone does not free the name: on Windows a replacement primary's `CreateFileMappingA` fails with `ERROR_ALREADY_EXISTS` until *every* worker has performed an operation past `primaryStaleMs`, so a supervisor must **retry** the restart rather than assume one attempt after the timeout will take. POSIX hides this by unlinking first. See §13.11. |
...
| 68 | **Writes go local-first, and the async variant differs only in what it awaits** | L3 first, so local tiers never hold what L3 rejected; a trailing write with no way to observe it | L3-first makes split state impossible, but its answer to an unreachable L3 is to write NOTHING -- not even locally -- which turns a remote outage into a local write outage. Local-first keeps the 45ns local write and keeps the cache serving through an outage, and the promise still carries the guarantee, so the caller who wants it can still wait. The window where local holds a value L3 has not accepted is bounded by the round trip, and other processes cannot see the local write until the primary drains the submission ring anyway -- documented at ~1 event-loop tick against 50-200us for a remote hop, so the window is mostly hidden. On failure the value is KEPT locally under a short TTL: serve through the outage, converge afterwards. The honest cost is that after that TTL a write reported as failed reverts to L3's older value. Background failures never touch `lastError` -- the sync call returned long ago -- so they go to stats and an optional listener. **The short TTL is now applied, rather than only documented.** A whole-branch review found `l3FailTtlMs` written in the constructor and read nowhere: the option, its `index.d.ts` entry, the failure matrix and two code comments all asserted a behaviour that did not exist, and without it a failed L3 write diverged from L3 permanently -- for the common `ttlMs: 0`, forever -- which is exactly the split brain the option was added to bound. It is applied on BOTH abandonment routes, because the failure matrix names both: an op that outlived `l3RetryMs`, and one shed on the spot past `l3QueueMaxBytes`. It SHORTENS ONLY. L1's deadline is lowered in place, and an entry that already expires sooner keeps its own. L2 has no retime operation, so the cap has to be re-applied by writing the value again -- and that is only safe while the arena still holds THIS value, so it is guarded by a compare against what is resident. If a later write superseded it, or a delete removed it, the cap is skipped: a local entry that outlives its cap costs a longer disagreement, while resurrecting an older value costs correctness, and *a miss is the failure mode this system is built around; a stale value is not*. Nothing is capped at `minLevel: L3`, where nothing was written locally to begin with. **"Converge afterwards" was false on a WORKER, on both abandonment routes.** A worker cannot write L2, so its write is still sitting in the submission ring when the L3 outcome arrives a microtask later -- and the compare above, asked then, looks at the arena's PREVIOUS value or at nothing, and correctly refuses. The compare is right and was asked too early: the result was that the value L3 refused sat in the SHARED arena with no expiry, forever, for every process on the box, while the primary's own path converged perfectly. So a worker keeps the cap in a small map and retries the compare on a 20ms unref'd poll until it is meaningful -- until the arena shows the value capped, shows something newer, or the window expires. The window is four maintenance backstop intervals (2000ms): a submitted write reaches L2 on the doorbell or, if that is lost, on the primary's 500ms backstop, and two such hops are needed (apply the write, then apply the cap). A write that has not landed within that is not going to, and there is nothing left to re-time. **And the worker's own reads converge at the deadline, not at the apply.** Between the cap falling due and the primary applying it, the capped L1 entry has expired and a read falls straight through to the uncapped L2 copy -- the worker serving, past its own deadline, the value L3 refused. While a cap is outstanding and past due, a read that finds exactly that value in L2 misses instead. Both checks are guarded on the map being non-empty, so a primary, and any cache with no adapter, pays nothing. The poll examines at most 64 caps per tick, because deciding whether one can be applied means asking the arena and that COPIES A VALUE OUT of it: a full pass over the 4096-entry bound was measured at 1.69ms for 8KB values, an event-loop stall every 20ms for the length of an outage. 64 holds it at ~26us and a full pass still completes in 1.28s. There is no confirmation phase: once a cap is submitted the arena has nothing left to tell us, because either it applies it and the entry expires on its own, or it does not and the read guard is what converges this worker -- watching for the confirmation cost a copy per entry per tick and, on a near-full map, reported caps that HAD been applied as unapplied. **A LOCAL WRITE CANCELS ITS CAP**, which a synchronous cap never needed. The cap decides by comparing against the ARENA, and a worker's write goes into the submission RING, so between the two there is a window in which the arena still holds the old value while the ring already holds the new one -- and a cap submitted in that window is ordered BEHIND the new write and re-applies the old value on top of it. That is the exact resurrection the compare exists to prevent, reached through this worker's own ring rather than through someone else's, and deferring the compare is what opened it. A write or a delete drops the cap outright, **and the drop lives inside the publish helpers rather than at the call sites**. "Cancel if and only if the write actually lands" is one sentence, and it was enforced by hand at nine sites -- the primary's set and del, a worker's submitSet and submitDel, the minLevel-3 eviction, the L3 promotion, three IPC-fallback outbox pushes. Four consecutive rounds of review each found one more of them on the wrong side of a branch: a cancel before a size check, a cancel before a shed test, a path that had lost its cancel entirely, a promotion that cancelled and then shed. Each was a resurrection or a value left in the shared arena with no expiry. That is what a hand-enforced invariant scattered over nine sites does, and the answer is structural: every route a write can take now ends in `#publishArenaSet/Del/Clear`, `#publishRingSet/Del` or `#publishOutbox`, the cancel sits inside their success branch, and `test/write_sites_test.js` attributes every raw call to the arena, the ring and the outbox to its enclosing method and fails if one appears anywhere else. **The IPC publish cancels even though it is only QUEUED**, and the accounting of that runs the opposite way from how it first reads: a cap that arrives LATE re-publishes with `ttlMs = cap`, so it clears itself within `l3FailTtlMs` and is bounded, while a cap that is LOST leaves the refused value resident with no expiry at all and is unbounded. So it trades an unbounded divergence for the certainty of never putting an old value back over a newer one -- chosen on this system's own ranking, where a miss is the accepted failure and a stale value is not, and not because the lost cap is the cheaper side. The cap's OWN write is the one exception and is named for it -- `#retimeArena`/`#retimeRing`/`#retimeOutbox` re-publish a value this process already published, with a shorter deadline, so they supersede nothing and must not cancel. **One member per transport, in both families.** The publish family had an arena route, a ring route and an IPC route while the retime family had only the first two, so on a `transport: 'ipc'` worker the cap had nowhere to go and this decision's whole promise was unkept: a value L3 had explicitly refused sat in the shared arena with no expiry, forever, and the only trace was an `l3FailTtlUnapplied` counter. Naming the families is what made that visible; the asymmetry had been there since the cap was written. Like `#publishOutbox`, the IPC retime is queued rather than confirmed, so a shed batch leaves the entry to time out on its window -- the same outcome as having no route at all, for the subset of cases where the batch is dropped. It cancels for EVERY INSTANCE in the process, not only the one that issued it: the caps are per-instance and the submission ring is per-process, so instance A's cap is ordered behind sibling B's write exactly as it would be behind A's own -- decision 64's family again, and the same sweep across `instances` that `#dropOthers` does. A PROMOTION is a write into the ring too, and cancels for the same reason; the route into that one is this very design, since the read guard is what makes a key with an outstanding cap miss locally and so sends the read to L3 to be promoted. **Another process's write is caught by the arena compare only when it comes from the PRIMARY**, which writes the arena synchronously. Another WORKER's write sits in a ring of its own, and nothing on this side can see it -- that interleaving is not closed, and the reason it is tolerable is the one this decision already rests on: the cap only ever SHORTENS, so the worst case is the arena holding a value for `l3FailTtlMs` rather than for its own TTL, never a value nobody wrote. **What comes back FROM L3 is untrusted in the same way.** An adapter's value used to be written into the shared arena BEFORE the decode that would have rejected it, so one bad reply poisoned a key for every process on the box: `get` threw the decode error on every read until the entry aged out. Validate and decode first, and treat an unusable value as an ordinary L3 miss -- reported through `onL3Error`, counted as `l3BadValues`, nothing stored. `rec.ttlMs` is untrusted too, and a naive `PTTL` passthrough answers -1 for "no expiry": that was truthy, min'd against `l3TtlMs` to -1, and handed to the arena, which stored the entry with NO expiry at all -- `l3TtlMs` bypassed by the very reply it exists to bound. Anything that is not a positive finite number now means "L3 named no deadline", and the upper end is clamped to `set()`'s uint32 range. **WAVE 4: TWO OF THE SENTENCES ABOVE WERE FALSE, AND THE SHAPE HAD TO CHANGE.** "Another process's write is caught by the arena compare only when it comes from the PRIMARY" was not true of the primary either: on a worker the compare and the apply are in DIFFERENT PROCESSES, separated by a hop, so anything the primary applied inside that hop -- a newer write, a delete -- was simply overwritten when the worker's re-write landed. A deleted key came back, or a newer value was replaced box-wide for `l3FailTtlMs`. And "the cap only ever SHORTENS" was true of the L1 half, which compares deadlines, and false of the L2 half, which never read the entry's remaining life at all: `set(k, v, {ttlMs: 2000})` whose L3 write failed at 1513ms had its L2 deadline rewritten from *now*, so the remaining life jumped back up and the value outlived what the caller asked for. **The cap is now a CONDITIONAL OPERATION THE PRIMARY APPLIES.** A worker may not write L2 at all (see decision 69's wave-4 amendment), so it sends `r` in the ordinary IPC batch, carrying the value the cap was taken against; `applyBatch` drains every submission ring to empty before applying it, so that worker's own write for the key has provably landed, and the primary then compares and writes in ONE synchronous step with nothing able to land between them. It writes only if the arena still holds exactly that value, and only if the entry's remaining life is longer than the cap -- both conditions readable synchronously, right there. A cap the primary refuses is not a failure and is not counted: nothing is left to bound. What stays on the worker is the READ GUARD alone -- "stop serving the value L3 refused at the deadline, rather than whenever the primary gets to it" -- so the poll no longer copies values out of the arena, needs no per-tick budget, and no longer retries anything. The honest cost of the new route is that a `transport: 'shm'` worker's cap now depends on the IPC channel being routed to `applyBatch`, exactly as `clearAll`'s generation ops already did; a batch shed by a congested channel is counted as `l3FailTtlUnapplied`, which is the same outcome the old shed retime had. **THE NEW ROUTE COUPLES A `shm` WORKER'S CAP TO `applyBatch`, AND THAT COUPLING FAILS SILENTLY UNLESS IT IS COUNTED.** `transport: 'shm'` moves the WRITES off the cluster channel; it does not take a worker off it -- a `clearAll`'s wipe and its two generation ops already travelled this way -- but a consumer that routes the ring doorbell and never calls `applyBatch` now loses the cap too, and the outcome is exactly the High finding this wave removed: the value L3 refused resident in the shared arena with NO EXPIRY, box-wide, forever. Worse, `l3FailTtlApplied` still moved (the L1 half always succeeds) and nothing moved on the other side, so the stats asserted success. So the silence is now a number in all four ways it can happen: no arena to write; the batch shed by a congested channel; the channel gone or refusing it; and -- the one nothing else can see -- a request that was SENT and never applied, detected when the read guard's window closes with the capped bytes still resident and still unbounded. That last check costs one arena read per cap ONCE, at retirement, not per tick, which is what the old poll did and why it needed a budget. It can over-report (someone rewriting the same bytes with no TTL after the cap landed), which is the right direction: the box really is holding that value unbounded. The coupling itself is stated where a hand-routing consumer actually lands -- `applyBatch`, `isCacheMessage`, `transport` (option and getter), `l3FailTtlMs`, and README's operational notes -- rather than only here. **The alternative was a new submission-ring op**, `SUBMIT_OP_RETIME`, so the cap would never leave shared memory. Rejected: it is a `TCS_LAYOUT` bump and a C++ change to the ring's validator and consumer, for a message sent once per FAILED L3 write -- traffic proportional to an outage, not to the write rate -- where the IPC path already exists, already carries `clearAll`'s ops, and already gives the ordering the compare needs (`applyBatch` drains every ring to empty first). The cost is one more thing a hand-wired primary must do; the mitigation is that it is documented in five places and counted when it is missed. **`stats.sent` counts the `r` entries**, deliberately: it has always counted every entry in an IPC batch, including the keyless clear ops, so it is a TRANSPORT counter -- messages handed to the primary -- and not a count of data writes, which is what `sets` and `deletes` are for. Splitting it would change an existing number's meaning to no operational end, since the cap already has `l3FailTtlApplied`/`Unapplied` of its own. **AND THE COUNTER HAD TO STOP GUESSING.** The first version inferred "never applied" from the capped bytes still being resident and unbounded at retirement. That is not sufficient: the primary can APPLY the cap, the entry can expire, and ANOTHER PROCESS can write byte-identical bytes with no TTL -- at which point the inference reports a fresh, L3-agreed write as a lost cap. A same-process rewrite cannot do this (`#cancelCaps` drops the guard), so the false positive was exactly cross-process, which is to say in the cluster the counter exists for. The invalidation ring is the oracle and it is already asked this very question by the promotion guard -- *did anyone else write this key since* -- so the cap records the ring head when it is taken and the retirement check reads forward from it. OUR OWN records are skipped by writer id, because the write being capped is itself one of them and would otherwise suppress every verdict this worker ever reaches. One ring pass per retiring batch, from the oldest mark, so 4096 caps retiring together cost one walk; the resulting hash set spans slightly more than any single cap's mark, which can only SUPPRESS a count, never invent one. **Two buckets, because the honest answer is sometimes "I cannot tell".** `l3FailTtlUnapplied` is now PROVEN divergence only -- nothing handed over, or handed over with the bytes still resident, still unbounded, and the ring saying nobody touched the key. `l3FailTtlUnconfirmed` is everything the process gave up on: a worker that degraded or closed mid-window, a ring that had lapped past the mark, and a cap past `L3_CAP_MAX` for which no guard entry could be kept at all -- that last one is why a sustained outage against a hand-wired primary used to UNDER-count from the 4097th outstanding cap onward, at exactly the scale where the number matters. A cap that is MOOT -- superseded by a newer published value, or wiped by a clear -- is in neither bucket, and those sites say so rather than leaving the omission to be rediscovered. Every one of these numbers now moves through a single function, which is what makes "one counting site" checkable rather than a claim; the `'unable'` return no longer increments behind its back. **AND A WRITER ID CANNOT IDENTIFY US, which nearly reopened the same hole one step further in.** The oracle skipped records bearing our own writer id, because the write being capped is one of them. But the primary stamps a shared-memory submission with the RING SLOT PLUS ONE and an IPC batch with the SENDER'S WRITER ID, and nothing keeps those spaces apart: a shm worker holding slot 0 compares against `mine = 1` and cannot tell its own record from one written by a `transport: 'ipc'` worker that was given id 1. Skipping it HID a genuine cross-process rewrite and reported a proven loss about somebody else's fresh write -- systematic in an oversubscribed or mixed-transport cluster. The fix is not a better skip but a THIRD ANSWER: a hash seen only on records carrying our own id is ambiguous, and the cap is counted `unconfirmed`. A transport tag on the record would have been exact and is a `TCS_LAYOUT` bump plus a C++ change to the validator and consumer, for a diagnostic counter; comparing on the pair needs the same field. Failing safe is what a counter whose whole value is being believed should do, and it costs the proven bucket only the caps whose own write landed INSIDE their window -- anything drained before the mark is not in the walk at all. **WHICH MEANS `unconfirmed` IS THE NORMAL BUCKET ON A BUSY BOX, and the documentation has to say so rather than leave an operator watching the quiet one.** The invalidation ring holds at most 65536 records -- on the order of 100ms of primary writes under load -- against roughly 7s from a cap's mark to its retirement at the default `l3FailTtlMs`, so the proof is usually unavailable for the ordinary reason that the ring has lapped. `index.d.ts` and README both say it. `index.d.ts` also no longer claims that every proven count means the value is RESIDENT: on a degraded worker, or where the shed batch carried the write beside the cap, there may be nothing in the arena at all. What is proven is that the cap did not land, which is the thing this number is for. **And "superseded means moot" is not unconditional.** `#publishOutbox` cancels a cap when the newer write is only QUEUED, and flush() can still shed that batch -- so a cap dropped as moot can turn out to have been superseded by nothing, leaving the refused value resident and in neither bucket. That trade is this decision's own and is made on its own ranking (a late cap bounds itself; a cap landing behind a newer write puts an old value back), but the "moot" claim must not be read as covering it, and the site says so. **AND THE WALK'S ROUND BOUND IS ITSELF AN ESCAPE HATCH UNLESS IT SAYS SO.** The bound was added for the reason `#primaryInvalidate` has one -- a producer faster than a synchronous reader must not keep it going -- but a bound that gives up and returns WHAT IT HAS is read by the caller as "no record for this key anywhere", so a rewrite sitting past the unscanned tail becomes a PROVEN loss: the same false fire the ambiguity answer exists to prevent, through the door the bound opened. An exhausted walk now returns `null`, indistinguishable from a lapped ring, and lands in `unconfirmed`. The bound is not tight by accident either: `ringCap` is capped at 65536 records and a round takes 1024, so 64 rounds cover a FULL ring and a static one always completes -- exhaustion therefore means only that the primary appended faster than the walk could read, sustained over a whole ring, which is exactly when the answer is unknown. That relationship is what the regression pins: it walks a nearly full ring, sixty-odd rounds deep, and requires a definite verdict. The exhaustion branch itself has no direct test, because reaching it needs a writer to outrun a synchronous reader by roughly twenty times for 65536 consecutive records; lowering the bound below the ring depth is how it was exercised, and with the `null` removed that same walk returns a proven verdict after seeing 8192 of 62000 records. **WAVE 5: THE CAP CONSULTED A MARK IT COULD NOT READ, AND ITS COMPARE WAS NOT A COMPARE.** `#capL2AfterL3Failure` answered `'moot'` -- *nothing left to bound* -- whenever `#pendingDel` held the key. That set also carried every `minLevel: 2` write (see decision 69's wave-5 amendment), so a worker's minLevel-2 write that L3 then REFUSED skipped the cap entirely: no request to the primary, no read guard, neither failure bucket, and `l3FailTtlApplied` still moving for the L1 half and reporting success, while the refused value sat in the SHARED arena with no expiry, box-wide. It is the ORDINARY case during an outage rather than a race -- a shed queue settles `false` in a microtask, and a real failure inside `l3RetryMs` beats a merely slow primary -- so once the queue backs up EVERY subsequent minLevel-2 write took it, and the failure-matrix row *Queue over `l3QueueMaxBytes` -> keep, capped at `l3FailTtlMs`* was simply false for them. The check is gone rather than corrected: a genuine pending DELETE is answered where it can be answered SYNCHRONOUSLY, by `#capArena` on the primary, after `applyBatch` has drained every submission ring to empty -- so the delete has provably landed and the compare says `'moot'` on its own. Asking a hop earlier could only guess. **AND `sameStored` WAS `===`.** Two value shapes are never equal to what the arena hands back although they round-trip through it perfectly: `NaN`, which is not `===` itself, and a string containing a lone surrogate, which UTF-8 cannot carry and which therefore comes back as U+FFFD. Both answered `'moot'`, so they were never capped and NOTHING was counted -- the silent direction. The compare is now on the STORED forms: unpaired surrogates folded the way the arena folds them, and two NaNs treated as the one value they are. `Object.is` was rejected for the NaN half because it splits -0 from 0, which the arena does not, and would have stopped capping a value that caps correctly today; the regression pins `neg0` alongside `nan` for exactly that reason. Every other type and every codec mode already round-tripped and is pinned too. LZ4 IS NOT BUILT IN THIS ENVIRONMENT, so the compressed path is reasoned rather than evidenced: compression is transparent to `native.get`, which decompresses before returning, so the compare sees the same raw form either way -- but no test ran it. |
| 69 | **A read marks the invalidation ring before it goes to L3, and refuses to promote if its key was invalidated meanwhile -- or if this process has already deleted it** | promote unconditionally; refuse to promote whenever the ring head moved at all; version every entry; for the pending delete, wait for the invalidation to come around, or serve L3's value and let the delete catch up | A read issued before a write can return the older value after it, and promoting that value overwrites the newer one in L2 -- not stale, WRONG, and it stays wrong until the next write to that key. The ring is already monotonic and `ringHead()` is already exported, so the mark costs one native call. Checking for the key's own hash rather than "did the head move" is what makes it usable: under load the head always moves, so the conservative version would never promote and L3 hits would never reach L2. **Three things block a promotion, not one.** Two of them are the ring answering *did anyone else change this key*: the key's own hash, and the flush marker, which is not a hash at all and means EVERYTHING changed -- a guard comparing only hashes lets a value a `clear` just removed straight back in, so both are fault-injected separately. `wrapped` falls back to conservative, which is correct and rare. The third asks a different question -- *did I already remove it* -- and the ring cannot answer it: on a worker the delete is applied by the primary a tick later, so during that window there is NO ring record to consult, which is exactly why the guard must consult `#pendingDel` itself rather than wait for the invalidation to come around. Reading through to L3 there returns the value the delete is on its way to removing, and promoting it sends that value back into L2 *after* the delete -- a resurrection every process in the cluster then sees, permanent until the next write. It is checked twice, before the request and after it, because a delete issued while the read was in flight is past the first check. **What the caller gets differs by reason, and the difference is the point.** A pending delete returns `undefined`: `get` already answers `undefined` for those keys, and the sync and async forms must not disagree about a delete this caller made and was told had succeeded. An unhashable key (a lone surrogate) still returns L3's value: L3 legitimately holds a key L2 cannot represent, `get`'s `undefined` there means "L2 cannot store this" rather than "this is deleted", and refusing the promotion costs residency, not correctness. `getAsync` is a superset of `get` by construction -- every L3 hit is a value `get` could not produce -- so the rule is not "they always agree", it is "they never disagree about this process's own state" -- **with one deliberate exception, not previously written down.** While a `minLevel: L3` write for a key is still queued or in flight, `get()` returns `undefined` (nothing was ever stored locally -- `minLevel: L3` evicts the local copies and never fills them) while `getAsync()` returns whatever L3 still holds, which is the value that write supersedes: the SET-outstanding branch below hands back `accepted.value` rather than `undefined`. That is not "this process's own state" disagreeing with itself by accident; it is the same "returning the value on a blocked promotion is deliberate" ruling two paragraphs down, applied to the one case where the blocked value and the caller's own in-flight write are for the same key. `l3_guard_test.js`'s `minLevel: L3` case is exactly this: `get('m')` is `undefined` while the concurrent `getAsync('m')` resolves `'v1'`, L3's pre-write value. **Three counters, so a dashboard stays readable:** `l3PromotionsBlocked` is contention -- this key changed while the read was in flight, or the ring cannot rule out that it did (degraded, detached, `wrapped`) -- and NOT a catch-all; `l3UnhashableKeys` is a key that can never live in L1 or L2 at all; `l3DeletedWhileReading` is a prevented resurrection. Folding the last two into the first would make the number an operator watches for promotions drying up unreadable. **There are FOUR block reasons, not three, and the fourth is the one this decision got wrong.** The third reason above -- *did I already remove it* -- was implemented only through `#pendingDel`, which exists only on WORKERS. The primary has none, because it applies a delete to the arena synchronously and has nothing to remember. So for one L3 round trip per delete the primary's local tiers were correct, the DEL was still on the wire, L3 still answered with the value, and a concurrent `getAsync` fetched it back and wrote it into the SHARED arena for a full TTL -- visible to every process on the box, after `delete` had returned true and `get` was already answering `undefined`. `delete` then read is an ordinary cache-aside sequence. The same hole covered a `minLevel: L3` write, which evicts the local copies and tells the adapter `willCache: false`: while its SET was on the wire a read promoted L3's PRE-write value into L1 and L2, where no provider would ever invalidate it, and reachable on workers too one drain later, when the ring clears `#pendingDel` while the L3 operation is still outstanding. **So the guard asks the L3 QUEUE what this process still owes L3 for the key, covering both the operation queued and the one in flight, and reporting the newest.** The queue answers a question the ring structurally cannot: the ring records changes that HAVE been applied, and an operation that has not reached L3 has by definition produced no record anywhere -- which is also why this check runs BEFORE the degraded-worker bail-out, since it holds whether or not there is an arena to read a ring from. It is one `Map` lookup on the read path, and free with no adapter, where there is no queue to ask. **This answers "what does this process still owe L3 for THIS KEY", not "is anything outstanding for this key at all" -- a `clear` does not live on a per-key chain and a key lookup cannot see it.** `clear` has no key (src/l3/queue.js's `CLEAR_ID`, a chain of its own), so `outstandingKind(key)` never returns `'clear'` for any key, however many are queued behind one. The question "is a clear on its way to L3 right now" is answered by an entirely separate mechanism -- `l3ClearsInFlight` (module-scope, this process) and the arena header's clear generation (cross-process, see decision 70) -- consulted a few lines below, not by this queue lookup. An outstanding DELETE returns `undefined` and counts `l3DeletedWhileReading`, for the reason this decision already gives for `#pendingDel`: `get` answers `undefined` for it, and the two forms must not disagree about this process's own state. An outstanding SET refuses the promotion and counts `l3PromotionsBlockedSelf` -- split out from `l3PromotionsBlocked` in wave 3b, for the same reason `l3UnhashableKeys` and `l3DeletedWhileReading` were split out rather than folded in: it is still returned what L3 holds, so it reads like the ordinary "someone else changed this key" case `l3PromotionsBlocked` counts, but the "someone" here is this same process's own write, not the ring reporting a change from elsewhere -- an operator watching `l3PromotionsBlocked` for contention on a hot key would otherwise see their own write traffic folded into it, unable to tell the two apart. **Returning the value on a blocked promotion is a deliberate choice, and the reason first given for it was wrong.** "L3 genuinely holds that value at that instant" is true of an outstanding SET of ours and false in general: when the block came from a ring HASH MATCH, the thing that changed the key may have been a removal that has already landed at L3, so L3 holds nothing at all. The right reason is narrower and survives that case: **nothing is stored when a promotion is blocked**, so there is no durable staleness anywhere -- the caller simply observes a value that existed when their read began, which is ordinary race semantics for any read that overlaps a write. The alternative is to return `undefined` whenever the guard blocked on a hash match for this key, which needs no layout change and would make the answer exact in the cross-process case too; it is rejected because it converts EVERY write-contended read of a hot key into a miss, which is a far broader cost than the narrow case it closes, and misses are the thing this system spends its design budget avoiding. The exactness gap is therefore recorded rather than closed: a read overlapping a removal made by a SIBLING INSTANCE or ANOTHER PROCESS has its promotion refused but is still handed the value. **And a BLOCKED promotion leaves a sibling's L1 untouched**, which is the same gap seen from decision 64's side: `#dropOthers` is reached on the paths that WRITE -- a promotion that lands, the `l3FailTtlMs` cap, an ordinary set or delete -- and a promotion that is refused writes nothing, so it drops nothing. A sibling instance holding a stale L1 copy of that key goes on serving it until something else invalidates it. That is the ordinary L1 contract rather than a new hole -- an L1 copy is only ever corrected by an invalidation, and a refused promotion produces none because nothing changed -- but it is worth stating, because the three call sites that DO sweep make it easy to read the family as "any L3 read fixes the siblings", which is not true. **And there are SIX.** The fifth belongs to decision 70 rather than to this one. The ring cannot answer "is a clear on its way to L3 right now" for the mirror-image reason the queue check exists: another process's clear puts its flush marker on the ring when the ARENA is emptied, which is before the adapter has applied it, so a read that starts after that marker sees a quiet ring, is handed the value the clear is still erasing, and writes it into the arena every process shares. `l3ClearedWhileReading` reads the arena header's clear generation, which is the only signal here that crosses a process boundary, and it joins `l3DeletedWhileReading` as a reason that changes the answer rather than only the placement. It also consults the process-local counter, because the header lags a worker by one flush and a read that worker already had in flight lands inside that lag. Named for the blocked read rather than for the gauge it consults: `stats.l3ClearedWhileReading` counts events, `native.l3ClearsInFlight()` measures a level, and one name for both would have made a dashboard lie. **The sixth is a removal of OURS that has already completed -- but completed after this read started.** Neither the queue nor `#pendingDel` can see it: the queue let the operation go when L3 acknowledged it, and the mark was cleared when the arena applied it. The ring still carries the record, but a hash match only says *this key changed*, which blocks the promotion and leaves the caller holding the value it deleted while `get` answers `undefined` -- the one disagreement this decision exists to forbid. **And the arena now publishes a removal it did not perform.** `storeDelete` appended to the invalidation ring only when it actually unlinked an entry, so a removal of a key the arena did not hold -- every `minLevel: L3` write of a new key, every delete of a key that lives only in L3 -- announced nothing. A worker's `#pendingDel` mark is cleared by that record, so those keys became unreadable from every tier including L3, permanently, and leaked toward the 4096-entry flush that then drops the real pending deletes. The record's meaning is now *this key's removal has been applied*, which is as true of an absent key as of a present one; `del()` still RETURNS whether anything was there. It costs one ring record per such delete, matching no worker's L1 index and so costing a hash compare rather than a refetch, and it is not a layout change. The consequence that needed handling is ring PRESSURE: a pure fan-out invalidation workload goes from zero ring traffic to full rate, and ~0.33s of that laps a 16MB arena's ring. So `#deletedAt` records where each of this instance's own removals landed on the ring, and the guard blocks when that position is later than the read's mark, counting `l3DeletedWhileReading` like the other two removal reasons. It is per-instance and bounded at 4096, populated only when an adapter is attached, and its positions are monotonic -- which is why a WRAPPED ring must not clear it: a wrap means "I do not know what changed", which is a reason to drop cached VALUES and never a reason to forget what this process itself removed. **But a wrap must RECONCILE those obligations, not merely keep them.** `#pendingDel` is cleared by the very record a wrap discards, so keeping the marks blindly left a delete the primary HAD applied marked forever, and that key then answered `undefined` from every tier including L3, with the adapter never asked -- the same defect the absent-key record was added to fix, reached through a different door and made more reachable by that record's own ring traffic. Discarding them blindly is the other half of the same mistake. So the wrapped branch asks the ARENA, which is the one authority a wrap does not destroy: a key it no longer holds is a removal that has been applied, so the mark is dropped and its position recorded in `#deletedAt` to keep an in-flight read refused; a key it still holds is a removal still outstanding, so the mark stays. One `has()` -- no value copy -- per mark, bounded by `PENDING_DEL_MAX`, on an event that is rare by construction. The residue is narrow and in the safe direction: a key deleted, applied, and then RECREATED by someone else inside the wrapped region reads as still held, so its mark survives and that worker misses on it until the next record for that key. `#deletedAt` and the cap map need no reconciliation -- the first is compared against ring POSITIONS, which are monotonic and which the guard refuses outright while the ring is wrapped, and the second resolves against the arena on its own timer, which a wrap tells it nothing about. **A KNOWN WINDOW, recorded rather than closed:** `#recovered()` clears `#pendingDel` wholesale, on the stated ground that a delete unapplied across a stall is a lost write rather than a pending one -- but the SUBMISSION RING can survive the stall, so a delete submitted before it may be applied after recovery with the mark already gone. That is a one-drain stale window, not a permanent one: the invalidation record for the applied delete arrives on the next drain and drops the L1 copy. Closing it properly means reconciling the ring against the arena at recovery, which is more machinery than a one-drain window is worth. **WAVE 4: A WORKER NO LONGER PROMOTES INTO L2 AT ALL.** Two consecutive reviews each closed one instance of a worker putting L3-derived bytes into the shared arena through the submission ring, and each time the next review found another door into the same room -- a promotion landing over the worker's own acked write, a failure cap re-writing an old value a hop later. Every one of them lived at the seam between an L3 reply and the ring's asynchrony: a worker's write is not in the arena when the call returns, and NOTHING orders it against the other work that writes the arena. So the rule is now structural rather than case-by-case: **only the primary may write L2 with L3-derived data**. A worker that reads through to L3 fills its own L1 and stops there. `#publishL3Derived` is the single route, it refuses any caller that is not the primary, `#fillFromL3` is its only call site, and `test/write_sites_test.js` fails if a write primitive or publish helper ever appears in the L3 read path again. The costs are named and accepted: each worker pays its own L3 read for the same key -- bounded by the worker count, the same bound this decision's `#inflight` sharing already accepts -- and while the primary is down nothing is promoted at all, which is a MISS, the failure mode this system is built around, unlike the stale and resurrected values the rule removes. **The LOCAL half of that first defect needed its own fix, and it is a FIFTH block reason.** A worker's `getAsync(k)` is in flight; the same worker awaits `setAsync(k, 'NEW')`, which lands in L1, is submitted to the ring, and is ACKNOWLEDGED BY L3. The L3 GET then answers from the pre-write value -- ordinary for a store that reads a snapshot -- and the promotion put the old value back into the worker's OWN L1, so the caller who had awaited `setAsync === true` read the old value from the instance it had just written through. None of the four reasons above can see it: the queue released the SET at the ack, `#pendingDel`/`#deletedAt` cover only removals, and the ring carries no record for a write the primary has not drained. An ordinary worker set therefore carries a self-mark, symmetric with what `#pendingDel` does for a delete, and it lives ON THE L1 ENTRY because it is worth exactly as long as the value it protects -- the invalidation record for our own write drops the entry and takes the mark with it, and an eviction takes both, where a separate map would outlive the value and, for a write the ring shed, outlive it forever and block every later promotion of that key. It is counted as `l3PromotionsBlockedSelf`, which is the same event this decision already names. **AND THE REASONS ARE ORDERED BY WHAT THEY CHANGE.** `owed === 'set'` was tested before both clear reasons, and only one of the two changes what the caller is TOLD: the self-set reason blocks the placement and still hands back what L3 returned, a clear answers `undefined`. So a read started before a `clearAll()` that happened to overlap a set of this process's own was handed the very value the clear was removing, while `get()` for the same key correctly said `undefined`. The guard must stop at the STRONGEST reason it matches, not the first. **WAVE 5: `#pendingDel` WAS AN OVERLOADED MARK, AND FOUR OF THE SEVEN FINDINGS THIS ROUND ARE ONE GUESS EACH.** A worker took that single mark for a DELETE (*I removed this key*) and for a `minLevel: 2` SET (*I wrote this key and it has not landed*). Those are different facts with different lifetimes, and every consumer had to guess which it was looking at: the cap read a set's mark as a pending delete and skipped itself (decision 68's wave-5 amendment); the wrapped-ring reconciliation keeps a mark while the arena still HOLDS the key, which is the removal's question and the exact inverse of a write's landing, so a wrap made a written key unreadable from every tier with L3 never asked; this guard reported a pending set as `l3DeletedWhileReading` and answered the caller `undefined` for a key it had just successfully written; and the drain handed a set to `#deletedAt`, which exists to refuse a read that would resurrect a REMOVED key. **So there are two mark sets, each naming what it holds.** `#pendingDel` and `#pendingWrite` are both `PendingMarks`, constructed with their kind, and the kind lives in the RECEIVER -- no site can take or release a mark without having chosen one. The one consumer that genuinely wants either (*can I read L2 for this key at all*) says so out loud in `#unappliedHere`, rather than inheriting it from an accident. `test/write_sites_test.js` is the choke point, as it is for the publish family: it fails if a mutation stops naming a kind, if a mark set is aliased into a local, if `#deletedHere` starts asking the write mark, if `#unappliedHere` stops asking both, or if the cap consults a mark again. **The wrap releases write marks and reconciles only removals**, because neither answer the arena can give justifies keeping a write mark: *held* is a write's landing, and *not held* leaves no older value for the mark to protect anyone from. The residue is stated at the site rather than left to be found: a write the primary has genuinely not drained, whose predecessor is still resident, can be read from L2 once after a wrap -- which needs a full ring (65536 records) appended while no submission ring is drained at all, i.e. the doorbell and the 500ms backstop both dead. Against that, keeping the mark is the permanent failure. **The L1 self-mark became a POSITION**, for the reason the removal mark already carried one: `own` was a boolean, so ANY record for the key's hash dropped the entry, including one written BEFORE our write and not yet drained here -- `set(k, NEW)` then `get(k)` on the same worker returned another process's older value, deterministically, whenever somebody else had written that key since this worker last drained. A record earlier than the entry's `ownAt` now leaves it alone. Every mark's position is read BEFORE its operation is published, never after: the primary can drain and append our record between the push and the read, and a mark stamped past its own record is one nothing can ever clear. **And a dropped IPC batch releases what it carried on ALL FOUR routes out, not just the shed one.** `process.send` throws SYNCHRONOUSLY for a value the serialiser cannot represent -- one `bigint` under JSON serialization does it for the whole batch -- and the async error callback and the disconnected-channel return are the same shape; each dropped the batch, counted `flushDropped`, counted caps, and left the marks, so a delete travelling beside a bigint left its key unreadable from every tier for the life of the worker. `#releaseBatchMarks` is the single route, keyed by op so each mark is released from the set that took it, and bounded by a mark SEQUENCE NUMBER taken with the batch, because a send can fail long after the worker has written the same key again and a newer mark belongs to a live submission. **AND THE PRIMARY'S PROMOTION IS NOW ORDERED AGAINST A WORKER'S UNDRAINED SUBMISSION.** Wave 4 made the primary the only writer of L3-derived data on the ground that its writes are ordered against its own -- they were ordered against a worker's submission by NOTHING. This guard walks the invalidation ring, and a submission the primary has not drained has no record there, so the primary promoted L3's pre-write value over a write a worker had already been told succeeded (and, in the delete-shaped variant, over a delete -- making the sync and async forms disagree). Its own promotion record then dropped the worker's self-marked L1 entry, so the worker read the old value back. The primary now drains every submission ring to empty before the walk, which is the same lever `applyBatch` already pulls before applying a cap and turns *no record exists yet* into *a record exists, at or past the mark*. **The IPC fallback has no such lever, and that is stated rather than closed:** a `transport: 'ipc'` worker's write lives in its outbox or in the channel until the primary's event loop delivers it, and nothing the primary can call makes that happen sooner -- so a write from such a worker can still be promoted over. The exposure is bounded where the shm one was not: the batch is delivered on the primary's next turn and `applyBatch` then writes the worker's value over the promotion, so it is a window of one hop rather than a value that stands until the next write. `shm` is the default transport and the only one where this is closed. |
| 70 | **`clear` empties every tier, and until it lands EVERY process sharing the arena serves misses** | clear only the local tiers; delete L3 keys by prefix in the background | With an adapter attached the tiers are ONE cache, so `clear()` that emptied only the local tiers would silently undo itself: the next read refills everything from L3. The queue cannot shed a clear and retries it indefinitely, because flushing twice is harmless while never flushing is not. While it is pending, L3 reads in this process return misses rather than the values the clear was meant to remove -- a miss is the failure mode this system is built around, a resurrected value is not. `delete` needs no such state: if L3 is unreachable, reads cannot refetch either, so the delete holds for the whole outage, and the only case where the old value returns is reads succeeding while the DEL failed, which the caller is told about -- **by `deleteAsync`'s promise, which resolves the L3 OUTCOME and not local presence.** It resolved presence, which made this clause false in both directions: a delete L3 refused resolved `true` while L3 still held the key, and an L3-only key whose delete landed everywhere resolved `false`. There is no local tombstone to inspect afterwards -- that is exactly what the clause before this one says -- and `delete` has already returned the local answer synchronously to any caller who wants it, so this promise is the only signal the design has left to give. With no adapter attached there is no remote half and it resolves presence, as before. `has` gets an optional adapter member because `EXISTS` is trivial for Valkey and not universal; without it the fallback reads the value and throws it away, which is correct and wasteful in a way the contract makes visible. **"This process" was the wrong scope, and a process-local flag of any kind could not have been right.** The pending state started as a per-instance boolean, was widened to a module-scope counter because several instances in one process share one arena (decision 64) -- and that was still a process-local answer to a cross-process question. The arena is shared, so while THIS process serves misses, another process reading the same L3 promotes the not-yet-cleared values into the same arena, and the clearing process is then served its own cleared value out of L2. Reproduced with a file-backed L3, a real primary and a real worker, in both directions. No refinement of a JS variable can fix it: the processes share memory and an arena, and nothing else. So the pending state lives in the arena header as two monotonic counters (`l3ClearGen`, `l3ClearSettled`), and the promotion guard gains a FIFTH block reason, `l3ClearedWhileReading`, which -- like `l3DeletedWhileReading` and for the same reason -- changes the ANSWER and not merely the placement: the clear already emptied L1 and L2 in every process, so a `getAsync` serving what L3 still holds would contradict both `get` and the entry guard that has been answering misses since the clear was issued. Only the primary may write the header (a worker's mapping is `PROT_READ`; a store there is a SIGBUS), so a worker's generation is opened and closed by ops in the IPC batch that already carries its `clearAll`, and the module-scope counter stays alongside the generation to cover the flush the header lags by -- and to be the only guard a degraded worker still has. `TC_LAYOUT` 6 -> 7: the counters are appended, so no existing field moves and the 64-byte rounding absorbs them (`sizeof(Header)` 264 -> 280, `indexOff` 320 either way), but a build made before this change would ignore them and promote over another process's clear, which is exactly what the gate exists to prevent. **The guard blocks reads of EVERY key, not only the ones a clear removed.** A clear has no key list -- that is what makes it a clear -- so while one is outstanding, every L3 read in every process on the box misses and every promotion is refused, for the whole round trip. That is the price of the guarantee and it is charged cluster-wide, so a workload that clears often pays it often; `clear` is an administrative operation, not a cache-shaped one. **A generation left open is therefore a cluster-wide outage of the L3 tier, and TWO routes could leave one open.** The obvious one is a worker killed mid-clear, which can never send its `-`. The likelier one needs no crash at all: `flush()` defers a whole batch while the IPC send window is full and sheds it once the outbox is full too, and nothing re-arms a deferred flush except the next write -- so the `-` of a worker that has just flushed its cache, and therefore has nothing more to say, could sit in the outbox forever or be thrown away with a batch of writes. Three things close them. A `-` is never shed with ordinary writes (it carries no key and no value, so keeping it cannot be what grows the outbox); while it is still queued an unref'd timer keeps retrying the flush, so it does not wait for traffic that may never come; and the primary keeps a per-writer count of generations opened and not settled, reconciling what a worker still owes when its channel emits 'exit' or 'disconnect' (`install()` wires this; `TurboKV.releaseWorker(message)` is public for a primary that routes cluster messages itself, exactly as `applyBatch` is). That count is also what makes `-` safe to accept at all: it is a lever that disarms a CLUSTER-WIDE guard, arriving over a channel any worker holds, so a worker may settle exactly as many generations as it opened and a `-` matching no `+` of its own changes nothing. **Reconciliation is keyed on the ATTACHMENT, not on the writer id.** `attachWorker` takes the id from the caller and a stable per-slot index is the ordinary way to name workers, so ids are REUSED across restarts: keying on one let a dead worker's reconciliation settle the clear of the live successor that took its slot, dropping the guard mid-clear and resurrecting exactly what that clear was removing -- no misuse required. Each process mints a nonce at load and carries it on its batches; the primary counts generations per nonce and `install()` reconciles a channel by naming it with one of its own messages, so a successor is a different attachment however early it attaches and the predecessor's `exit` can arrive whenever it likes. **A clearAll is not shed under congestion either.** The keep-list that saves `+`/`-` from the shed path saves `c` with them, for the same reason and by the same argument (no key, no value, one per clearAll): losing the `c` loses the clear itself while its bookkeeping reports success -- the guard opens and cleanly settles while L2 goes on serving values L3 no longer has. A batch that names no attachment gets NO generation -- falling back to the writer id would re-open this very finding during a mixed-version rolling restart, which is the same stable-slot deployment -- while the `c` in it still applies, because a wipe can only remove data and refusing it would leave L2 serving what that worker's L3 clear removes. `releaseWorker` takes a message and THROWS on a writer id: that call used to work and would now settle nothing, and a silently leaked generation is a cluster-wide L3 outage. `install()` keeps `{id, n}` rather than the message, so reconciliation does not pin a worker's largest batch for the life of its channel. **Two residues, recorded rather than fixed.** The accounting is keyed on an identity the SENDER declares, so it closes the unpaired disarm -- a `-` with no `+` now does nothing -- but does not bind a generation to the channel it arrived on: a worker could still name another worker's attachment, which is the same trust domain that already lets any worker send `c`. And settling on `disconnect` disarms the guard while that worker's clear may still be landing in L3, so a graceful rolling restart opens a real resurrection window; the alternative, waiting for `exit`, holds the guard armed for a worker that can no longer close it, and neither is free. A clear that never lands keeps the guard armed for as long as it is genuinely outstanding, which is correct -- L3 really does still hold what it was told to remove; `close()` settles it, by the same promise chain that decrements the counter. **A WORKER'S `clearAll()` IS ASYNCHRONOUS FOR EVERYTHING BUT ITS OWN L1, and that is recorded rather than fixed.** A worker cannot write L2, so the wipe travels to the primary as an op in the IPC batch: the arena, every other process's L1, and any SIBLING INSTANCE'S L1 in this same process are emptied when the primary applies it -- typically the next event-loop turn, but not before `clearAll()` returns. In that window a sibling handle can still serve a value the clear is removing, and a write issued after the clear can be ordered before it in the arena. Pre-existing, inherent to "the primary is the sole writer", and bounded by one turn; `index.d.ts` now says so on `clearAll` itself rather than leaving a caller to discover it. The L3 half is unaffected -- `l3ClearsInFlight` is bumped synchronously in the calling process and the arena generation covers the others -- so no process serves a value the clear removed *through L3*; what is visible in the window is L2's own copy, which the wipe then removes. |
| 71 | **`close()` returns a promise, and the wait it does is bounded** | leave it synchronous and let the adapter leak; a separate `shutdown()`; await the L3 queue's drain unconditionally | An open L3 connection keeps Node's event loop alive, so a synchronous `close()` would leave a process that cannot exit -- the kind of defect that shows up as a hung CI job rather than a failing test. Draining the queue first is part of the same obligation: work already accepted from a caller should reach L3 before the process goes away. Widening the return type from `void` to `Promise<void>` is backwards compatible for every caller that ignores it, which is the normal case, and a second method would leave `close()` as a trap. **The drain wait is bounded, and that bound is load-bearing, not cosmetic.** A `clear` is never shed by the queue and retries indefinitely by design (decision 70): until it lands, this process must serve misses rather than the values the clear was meant to remove. So if L3 is unreachable when `close()` is called, the queue's pending count never returns to zero and `drain()` never resolves -- awaiting it unconditionally would hang `close()` forever during exactly the outage where an operator most wants the process to exit. `l3CloseTimeoutMs` caps the wait (default 5000ms, the same window `l3FailTtlMs` already uses elsewhere in this design; 0 opts out and waits without a bound, for a caller that would rather hang than risk dropping work) and on expiry `close()` proceeds anyway: a caller that asked to close gets to close. **The honest cost of that bound:** whatever is still queued when it expires does not get cancelled -- it keeps retrying in the background, against an adapter this process is about to hand its own `close()` to, and it gets no second chance to be waited on. It either lands silently with nothing left to observe the outcome, or it exceeds `l3RetryMs` and is abandoned and reported through the normal `onL3Error`/stats path (moot for a `clear`, which never abandons on its own), or it is still retrying when the process itself exits and the outcome is simply never known. A caller closing during a known outage who needs a stronger guarantee than "waited up to N seconds" has to get it some other way -- there is no way to make waiting on an unreachable peer both bounded and complete. **The bound covers the adapter's own `close()` too, separately from the drain.** A first pass bounded only the queue's `drain()` and left `await this.#l3.close()` unbounded on the theory that "close" is a fast local teardown -- wrong in general: a real client's `close()` can itself wait on a graceful-shutdown handshake (in-flight commands draining, a TLS close_notify) that never completes if the peer is the thing that is down. `close()` therefore races the adapter's `close()` against the same `l3CloseTimeoutMs` ceiling, as a second, independent bound after the drain's. Worst case this makes `close()` take roughly 2x `l3CloseTimeoutMs` (once per bounded step), which is the honest price of a guarantee that covers both places an adapter can hang rather than only the one this task started with. The implementation order matters too: the L3 half runs *after* the existing synchronous teardown (ring release, primary arena destroy), not before it. An earlier draft put it first, which is a real defect this task's own tests caught -- every pre-existing caller in this codebase invokes `close()` without awaiting it, and delaying the synchronous teardown behind an `await` left the arena undestroyed when the very next test opened a new one, corrupting unrelated tests. The L3 shutdown never touches arena state (queued writes go straight to the adapter, not through L2), so nothing requires it to run first, and running it last keeps every existing fire-and-forget `close()` caller working exactly as before. `close()` also clears `#inflight`, the herd-sharing map for concurrent `getAsync` reads: a read stuck against a hung adapter connection is not cancelled by this either (nothing here can reach into the adapter to stop it), but clearing the map drops this instance's own reference so a stalled read cannot keep a cache `close()` has already torn down reachable for the life of that hang. **`close()` is idempotent.** It is exactly the method that gets called from more than one place -- a shutdown hook, a signal handler, a test's own teardown -- and two of those firing is ordinary, not exotic; an independent review confirmed a second call re-invoked `adapter.close()` and re-ran the drain, unguarded. The public `close()` now only memoizes and returns a private `#closePromise`, produced by `#doClose()` (the method that used to be `close()` itself); every call after the first returns that same promise rather than a fresh one, so two callers awaiting `close()` from different places observe the real outcome together rather than one getting a synthetic "done" ahead of the other. **Bounding the WAIT was not enough to bound the PROCESS.** A whole-branch review verified `clearAll(); await close()` against a failing `clear`: `close()` returned, honouring every bound above, and the process then hung until it was killed. The bounds made `close()` return; they did not make the queue stop. A `clear` is never shed and retries indefinitely (decision 70), and its retry backoff was a ref'd `setTimeout` that went on rescheduling itself for the life of the process -- against a cache that no longer existed. The same hung CI job this decision exists to prevent, reached by a different route. Two changes, and neither substitutes for the other: `close()` tells the queue it is CLOSED, so the retry loop ends at its next attempt instead of retrying forever; and the sleeping backoff is UNREF'D at that moment, so the wait already in progress stops holding the event loop immediately rather than one backoff later. The backoff is unref'd at close rather than at creation deliberately -- an unconditionally unref'd backoff lets a process with nothing else ref'd exit MID-RETRY, so `await setAsync(k, v)` against a failing L3 never resolves and accepted work is dropped with no caller left to hear about it (measured: the probe exits 0 before its own `await` returns). The flag is set AFTER the bounded drain, so work already accepted still gets its bounded chance to land. **`withDeadline`'s timer had the defect this paragraph describes, and reintroduced it from the other end of the same lifecycle.** It was unref'd at creation, with the same rationale that was rejected for the backoff -- a bound against a hang must not itself stop the process exiting -- and the same measurement refutes it: against an adapter that HANGS rather than throws, with nothing else ref'd, the process exits 37ms into a 300ms deadline, so `await setAsync` never settles, never prints, and exits 0 as though it had succeeded. The hang route has no backoff to be held open by, because the operation never reaches the catch that would start one. So both kinds of timer now live in one registry (`ServiceTimers`): ref'd while the queue is in service, unref'd wholesale by `close()`, and unref'd on arrival if armed after close. That is the whole rule, stated once, for the backoffs and the deadlines alike -- and the read path in `turbokv.js`, which has no queue chain of its own, takes its deadlines from the same registry so a hung `getAsync` cannot detain a cache that has closed. **The guarantee is now tested as a guarantee**: a child process sets up a failing `clear`, closes, and must exit on its own within a bound, with the parent's kill timer turning a regression into a failed assertion rather than a wedged CI job -- nothing inside a process can assert that it exited. One more thing the same review found here: **`close()` must never reject.** Its synchronous teardown can throw (`stopGuard()`, the ring-release loop, the unwrapped `native.destroy()`), and widening the return type turned every one of those throws into a rejection of a promise that every caller deliberately ignores -- an unhandled rejection, which Node 18 turns into process termination. A failure to release a ring slot would have killed the process it was cleaning up. The teardown and the L3 half are now caught separately and reported through `lastError` and `onL3Error`'s already-declared `close` kind, and `close()` resolves either way. |
...
| 56 | **The package is `turbokv`, and the class is `TurboKV`** | `turbocache` (previous name); `@krassx/turbocache`; keep the class as `TurboCache` under a `turbokv` package | The name `turbocache` is taken on npm -- a 139-byte proprietary placeholder published in 2020 by an unrelated author and untouched since 2022 -- so `npm publish` would have failed with a 403. That was missed when the publish workflow was built, which is the wrong order: check the name is claimable before building a pipeline to claim it. `turbokv` was verified available against the live registry, along with the rest of the shortlist. A scoped `@krassx/...` name was the zero-risk alternative and was rejected only because an unscoped name is claimable here and reads better in an install line; `turbo-cache` was rejected for sitting adjacent to Vercel's `turbo*` trademark in the same ecosystem. The class was renamed with the package rather than left mismatched: there are no published consumers, so this was the only free moment, and `Cache` remains as an alias for anyone who prefers a neutral name. The addon target moved too, so the shipped binary is `turbokv.node` rather than a file named after a different package. Deliberately NOT renamed: the `MSG = 'tc'` IPC tag and the `/tc-` arena-name prefix are wire and shared-memory identifiers rather than branding -- the prefix is length-constrained and changing it would need a `TC_LAYOUT` bump for no benefit. 62 files, verified by the full suite, both native tests, the declaration type-check (which self-references the package through the `exports` map, so it proves the new name resolves on both the CJS and ESM conditions) and the 95% coverage gate. **AMENDED: the published package is `@krassx/turbokv`.** "Verified available against the live registry" was the wrong test, and this decision recorded it as though it settled the question. A 404 says nobody holds the name; it does not say npm will let you take it. npm also refuses a name that is too SIMILAR to an existing one, normalising punctuation away before it compares -- and that check exists nowhere but the registry's PUT, so it fires after the tarball has been built, uploaded and accepted for upload: `403 Package name too similar to existing package turbo; try renaming your package to '@krassx/turbokv'`. The same rule kills the obvious retreats: `turbo-kv` normalises to exactly `turbokv`, and `turbo-cache` to `turbocache`, which is the taken name this decision started from. A scoped name is not subject to the similarity check at all, which is why it is the one option that could be chosen with certainty rather than attempted -- and it was already the alternative this decision listed and rejected for reading better in an install line. It keeps everything else intact: the class stays `TurboKV`, the repository stays `turbokv`, the addon stays `turbokv.node`, and `Cache`, `MSG = 'tc'` and the `/tc-` prefix are untouched. `turbo-kv` was considered and refused for a second, independent reason this decision already gave: it sits adjacent to Vercel's `turbo*` trademark in the same ecosystem, which is why `turbo-cache` was rejected above. Self-reference through the `exports` map is what proves the rename: the declaration type-check imports the package BY NAME on both conditions, so it fails if the new name does not resolve. |
...
7a. **A worker's `clearAll()` is asynchronous for everything but its own L1.**
   A worker cannot write L2, so the wipe travels to the primary as an op in the
   IPC batch: the arena, every other process's L1, and any *sibling instance's*
   L1 in this same process are emptied when the primary applies it — typically
   the next event-loop turn, but not before `clearAll()` returns. In that window
   a sibling handle can still serve a value the clear is removing, and a write
   issued after the clear can be ordered before it in the arena. Inherent to
   "the primary is the sole writer", bounded by one turn, and recorded rather
   than fixed: closing it would mean either a synchronous cross-process wipe a
   worker has no way to perform, or making `clearAll()` async, which is a
   breaking change to a synchronous method for a window one turn wide. The L3
   half is *not* affected — `l3ClearsInFlight` is bumped synchronously in the
   calling process and the arena generation covers the others — so no process
   serves a value the clear removed *through L3*. `index.d.ts` says so on
   `clearAll` itself.
