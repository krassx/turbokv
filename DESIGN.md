...
21. ~~**The primary served stale L1 across instances in the same process.**~~
    **Fixed** — decision 64. This was live in the published code path, not a
    theoretical gap: `set()` and `delete()` on the primary skip the ring record
    for their own write (`writerId === 0`), so a second `TurboKV` instance
    opened in the same process never saw the invalidation and kept serving the
    stale value, including a deleted one, until eviction. Pinned by
    `instances_test.js`, verified to fail with the fix reverted.
